    ...
    <div class="settings-card">
        ...
        <div class="tag-list">
            <?php foreach($shapes as $k => $v): ?>
                <div class="tag" style="border-color: #3498db;"> <?php echo $v; ?>
                    <?php if($role == 'admin'): ?>
                    <form method="POST" style="display:inline;">
                        <input type="hidden" name="target_file" value="shapes.json">
                        <input type="hidden" name="item_key" value="<?php echo $k; ?>">
                        <button type="submit" name="action" value="delete" class="btn-del-tag" onclick="return confirm('Delete shape: <?php echo $v; ?>?')">×</button>
                    </form>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
