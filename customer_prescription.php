...
<html>
    ...
    <body>        
        ...
        <script>
            ...
            // Build HTML for one eye from AI response
            function buildEyeFromAI(eyeLabel, eyeData) {
                let html = `<h4>◉ ${eyeLabel}</h4>`;
                if (!eyeData || !Array.isArray(eyeData.conditions)) {
                    html += `<p class="analysis-text">No data available for this eye.</p>`;
                    return html;
                }
                if (eyeData.summary) {
                    html += `<p class="analysis-text" style="font-style: italic; color: #aaa; margin-bottom: 12px;">${escHtml(eyeData.summary)}</p>`;
                }
                ...
                if (analysis.contextual_insights) {
                    summaryHtml += `
                        <div class="analysis-insights">
                            <h4>✦ CONTEXTUAL INSIGHTS (PERSONALIZED)</h4>
