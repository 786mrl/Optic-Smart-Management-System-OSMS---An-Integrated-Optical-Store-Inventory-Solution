<?php
    ...
    // DATA SAVE PROCESS
    if (isset($_POST['submit_customer_prescription'])) {
        // 1. Get Basic Data
        ...
        $time_val  = strtotime($exam_date);
        $month_rom = getRomawi(date('n', $time_val));
        $year_val  = date('Y', $time_val);
        // Retrieve the latest entry from the examination_code column
        ...
        if (mysqli_num_rows($res_seq) > 0) {
            ...
            $last_code = $last_row['examination_code'];
            
            // Explode the string LZ/EC/001/II/2026 based on the '/' delimiter
            $parts = explode('/', $last_code);
            
            // Get the sequence part (index 2) and increment by 1
            $sequence = (int)$parts[2] + 1;
        } else {
            $sequence = 1; // If database is empty
        }
        // Format sequence to 3 digits (e.g., 001, 002)
        $seq_padded = str_pad($sequence, 3, '0', STR_PAD_LEFT);
        // Combine into Final Code
        $exam_code = "LZ/EC/$seq_padded/$month_rom/$year_val";
        ...
        // 6. Insert Query
        $sql = "INSERT INTO customer_examinations (
            ...
            '$exam_date', '$exam_code', '$customer_name', '$gender', $calculated_age, '$symptoms_to_save',
            '$old_r_sph', '$old_r_cyl', '$old_r_ax', '$old_r_add',
            '$old_l_sph', '$old_l_cyl', '$old_l_ax', '$old_l_add',
