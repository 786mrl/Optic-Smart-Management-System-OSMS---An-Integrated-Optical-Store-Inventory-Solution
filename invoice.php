...
<html lang="en">
        ...
        <style  id="mediapipe-styles">
            ...
            * { box-sizing: border-box; }
            ...
            /* ================================================================
               ...
               Sembunyikan semua UI biasa, tampilkan hanya #print-invoice
               ================================================================ */
            @media print {
                body > *,
                .main-wrapper,
            ...
            <div class="content-area" style="flex-direction: column">
                ...
                width: 100%;">
                    ...
                    <div style="margin-top: 40px; text-align: center;">
                        <button onclick="openPrintInvoice()" class="btn-action">PRINT INVOICE</button>
                    </div>
                </div>
            </div>
    ...
    <!-- ================================================================
         PRINT INVOICE SHEET
         Tersembunyi di layar, muncul saat window.print() dipanggil.
         ================================================================ -->
    <div id="print-invoice" style="display:none;">
        <style>
            ...
            #print-invoice .inv-header-info { text-align: right; }
            ...
            #print-invoice .inv-meta { text-align: right; }
            ...
            #print-invoice .inv-cust-item { display: flex; flex-direction: column; gap: 2px; }
            #print-invoice .inv-cust-item.full { grid-column: 1 / -1; }
            ...
            #print-invoice .inv-items { display: flex; flex-direction: column; gap: 5px; }
            ...
            #print-invoice .inv-payment-row.total-row .lbl { color: #c9a84c; font-size: 7.5px; }
            #print-invoice .inv-payment-row.total-row .val { color: #c9a84c; font-size: 11px; }
            #print-invoice .inv-payment-row.balance-row .val { color: #ff8a65; }
            #print-invoice .inv-payment-row.balance-row.paid .val { color: #69f0ae; }
            ...
            #print-invoice .inv-habit-row { display: flex; gap: 5px; flex-wrap: wrap; margin-top: 7px; }
            ...
            #print-invoice .inv-habit-chip.habit { background: #e8f5e9; border: 1px solid #81c784; color: #2e7d32; }
            #print-invoice .inv-habit-chip.digital { background: #e3f2fd; border: 1px solid #64b5f6; color: #0d47a1; }
            #print-invoice .inv-contact-row { display: flex; gap: 6px; flex-wrap: wrap; margin-top: 7px; }
            ...
            #print-invoice .inv-sign-block { display: flex; flex-direction: column; align-items: center; gap: 3px; }
            #print-invoice .inv-sign-line { width: 80px; border-top: 1px solid #0d0d0d; margin-top: 22px; }
            ...
            #print-invoice .inv-sheet::before {
                content: '';
                position: absolute;
                top: 0; left: 0;
                width: 0; height: 0;
        ...
        <div class="inv-sheet">
            ...
            <div class="inv-body">
                ...
                <div>
                    ...
                    <!-- Habit chips -->
                    <div class="inv-habit-row">
                        <?php
                            $habitLabels   = [1 => '👁 Indoor', 2 => '☀️ Outdoor', 3 => '🌍 Indoor & Outdoor'];
                            $digitalLabels = [1 => '💻 Low Screen', 2 => '💻 Moderate Screen', 3 => '💻 High Screen'];
                            $hv = (int)($data['visual_habit']  ?? 1);
                            $dv = (int)($data['digital_usage'] ?? 1);
                        ?>
                        <span class="inv-habit-chip habit"><?php echo $habitLabels[$hv] ?? 'N/A'; ?></span>
                        <span class="inv-habit-chip digital"><?php echo $digitalLabels[$dv] ?? 'N/A'; ?></span>
                    </div>

                    <!-- Phone & Address — diisi JS sebelum print -->
                    <div class="inv-contact-row" id="inv-contact-display"></div>
                </div>
                    ...
                    <table class="inv-rx-table">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars(($data['lens_modification']==1 && !empty($data['mod_r_sph'])) ? $data['mod_r_sph'] : ($data['new_r_sph'] ?: '—')); ?></td>
                                <td><?php echo htmlspecialchars(($data['lens_modification']==1 && !empty($data['mod_r_cyl'])) ? $data['mod_r_cyl'] : ($data['new_r_cyl'] ?: '—')); ?></td>
                                <td><?php echo htmlspecialchars(($data['lens_modification']==1 && !empty($data['mod_r_ax']))  ? $data['mod_r_ax']  : ($data['new_r_ax']  ?: '—')); ?></td>
                                <td><?php echo htmlspecialchars(($data['lens_modification']==1 && !empty($data['mod_r_add'])) ? $data['mod_r_add'] : ($data['new_r_add'] ?: '—')); ?></td>
                            </tr>
                            <tr>
                                <td class="eye-cell">L</td>
                                <td><?php echo htmlspecialchars(($data['lens_modification']==1 && !empty($data['mod_l_sph'])) ? $data['mod_l_sph'] : ($data['new_l_sph'] ?: '—')); ?></td>
                                <td><?php echo htmlspecialchars(($data['lens_modification']==1 && !empty($data['mod_l_cyl'])) ? $data['mod_l_cyl'] : ($data['new_l_cyl'] ?: '—')); ?></td>
                                <td><?php echo htmlspecialchars(($data['lens_modification']==1 && !empty($data['mod_l_ax']))  ? $data['mod_l_ax']  : ($data['new_l_ax']  ?: '—')); ?></td>
                                <td><?php echo htmlspecialchars(($data['lens_modification']==1 && !empty($data['mod_l_add'])) ? $data['mod_l_add'] : ($data['new_l_add'] ?: '—')); ?></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div>
                    ...
                    <div class="inv-items" id="inv-items-list">
                        <div class="inv-item-placeholder">— Belum ada item dipilih —</div>
                    </div>
                </div>
            ...
            <div class="inv-footer">
                <div>
                    <div class="inv-footer-msg">Terima kasih atas kepercayaan Anda. Simpan invoice ini sebagai bukti pembelian.</div>
                </div>
                <div style="display:flex;gap:20px;align-items:flex-end;">
                    <div class="inv-sign-block">
                        <div style="font-size:6px;color:#aaa;letter-spacing:1px;font-family:Arial,sans-serif;font-weight:700;">CUSTOMER</div>
                        <div class="inv-sign-line"></div>
                        <div class="inv-sign-label">Tanda Tangan</div>
                    </div>
                    <div class="inv-sign-block">
                        <div style="font-size:6px;color:#aaa;letter-spacing:1px;font-family:Arial,sans-serif;font-weight:700;">PETUGAS</div>
                        <div class="inv-sign-line"></div>
    ...
    <script>
    /* ── openPrintInvoice(): kumpul data dari UI → isi print sheet → print ── */
    function openPrintInvoice() {
        ...
        if (contactDiv) {
            var chips = '';
            if (phone)   chips += '<div class="inv-contact-chip"><span class="chip-label">PHONE</span><span class="chip-value">' + _esc(phone) + '</span></div>';
            if (address) chips += '<div class="inv-contact-chip"><span class="chip-label">ADDRESS</span><span class="chip-value">' + _esc(address) + '</span></div>';
            contactDiv.innerHTML = chips;
        }

        /* 2. Order items — baca dari #lr-selection-bar-inner */
        var itemsList = document.getElementById('inv-items-list');
        if (itemsList) {
            var rows = '';
            var innerEl = document.getElementById('lr-selection-bar-inner');
            if (innerEl) {
                /* Cari setiap item card di dalam selection bar */
                var cards = innerEl.querySelectorAll('[data-inv-type]');
                if (cards.length) {
                    cards.forEach(function(card) {
                        var type  = card.getAttribute('data-inv-type')  || '';
                        var name  = card.getAttribute('data-inv-name')  || '';
                        var price = card.getAttribute('data-inv-price') || '';
                        rows += _itemRow(type, name, price);
                    });
                } else {
                    /* Fallback: ambil teks mentah dari inner */
                    var lines = innerEl.innerText.trim().split('\n').filter(function(l){ return l.trim(); });
                    lines.forEach(function(line) {
                        if (line.trim()) rows += _itemRow('', line.trim(), '');
                    });
                }
            }
        ...
        _setTxt('inv-pay-total',   totalEl   ? (totalEl.value   || '—') : '—');
        _setTxt('inv-pay-paid',    paidEl    ? (paidEl.value    || '—') : '—');
        _setTxt('inv-pay-balance', balanceEl ? (balanceEl.textContent || '—') : '—');
        _setTxt('inv-due-date',    dueEl     ? (dueEl.textContent.replace('— select a lens','—').trim() || '—') : '—');

        /* 4. Balance colour: hijau jika lunas */
        var balRow = document.getElementById('inv-pay-balance-row');
        if (balRow && balanceEl) {
            var num = parseInt(balanceEl.textContent.replace(/[^0-9]/g,'') || '0', 10);
            num === 0 ? balRow.classList.add('paid') : balRow.classList.remove('paid');
        }
    ...
    function _itemRow(type, name, price) {
        return '<div class="inv-item-row">'
             + '<div><div class="inv-item-type">' + _esc(type) + '</div>'
             + '<div class="inv-item-name">' + _esc(name) + '</div></div>'
             + (price ? '<div class="inv-item-price">' + _esc(price) + '</div>' : '')
             + '</div>';
    }
