<?php
    ...
    if (isset($_POST['create_direct_sale'])) {
        ...
        $ds_notes    = mysqli_real_escape_string($conn, 'DISPENSING ONLY');
    ...
    if (isset($_POST['save_direct_sale'])) {
        ...
        $ds_zero = '0.00'; $ds_zero_ax = '0'; $ds_va = '20/20';
        $ds_notes = mysqli_real_escape_string($conn, $p['exam_notes'] ?? 'DISPENSING ONLY');
    ...
    if ($is_direct_pending) {
        ...
        $data = [
            ...
            'ucva_r'            => '20/20',
            'ucva_l'            => '20/20',
            'exam_notes'        => $p['exam_notes'] ?? 'DISPENSING ONLY',
...
<html lang="en">
    <head>
        ...
        <style>
            ...
            /* Center the header-container block itself on the page */
            .header-container {
                margin-left: auto;
                margin-right: auto;
                width: fit-content;
    ...
    <body>
                    ...
                    <form method="POST" action="invoice.php">
                        ...
                        <div class="ds-card ds-collapsible collapsed" id="ds_card_name" onclick="dsCardToggle(this, event)">
                            ...
                            <div class="ds-card-body" onclick="event.stopPropagation()">
                                ...
                                <div class="ds-done-row">
                                    <button type="button" class="ds-done-btn" onclick="dsUpdateNameSummary(); dsSectionDone('ds_card_name', 'ds_card_gender')">DONE</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="ds-card ds-collapsible collapsed" id="ds_card_age" onclick="dsCardToggle(this, event)">
                            ...
                            <div class="ds-card-body" onclick="event.stopPropagation()">
                                ...
                                <div class="ds-done-row">
                                    <button type="button" class="ds-done-btn" onclick="dsProcessAge()">DONE</button>
                                </div>
                            </div>
                        </div>
        ...
        <script>
            ...
            // Shared age-processing logic: parses the age/birth-year input, updates summary,
            // auto-fills ADD (complete purchase only), shows/hides vision need, and advances the card.
            // Used by both the input's blur event and the card's DONE button.
            function dsProcessAge() {
                var input = document.getElementById('ds_age');
            ...
            function dsSectionDone(currentId, nextId) {
                ...
                if (nextId) {
                    ...
                    if (next) {
                        dsExpandCard(next);
                        dsScrollCardIntoView(next);
                    }
                }
            }

            // Auto-scroll the newly opened card into view (waits for the expand transition to finish
            // so the card's final height is used when centering — otherwise it centers too early)
            function dsScrollCardIntoView(card) {
                if (!card) return;
                setTimeout(function() {
    ...
    <body style="background: var(--bg-color);">
                                            ...
                                            <table class="prescription-table">
                                                ...
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        <td><input type="tel" inputmode="numeric" name="od_sph" class="input-table-neu mod-field" 
                                                        ...
                                                        <td><input type="tel" inputmode="numeric" name="od_cyl" class="input-table-neu mod-field" 
                                                        ...
                                                        <td><input type="tel" inputmode="numeric" name="od_axis" class="input-table-neu mod-field" 
                                                        ...
                                                        <td><input type="tel" inputmode="numeric" name="od_add" class="input-table-neu mod-field" 
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><input type="tel" inputmode="numeric" name="os_sph" class="input-table-neu mod-field" 
                                                        ...
                                                        <td><input type="tel" inputmode="numeric" name="os_cyl" class="input-table-neu mod-field" 
                                                        ...
                                                        <td><input type="tel" inputmode="numeric" name="os_axis" class="input-table-neu mod-field" 
                                                        ...
                                                        <td><input type="tel" inputmode="numeric" name="os_add" class="input-table-neu mod-field" 
                        ...
                        <div class="full" id="mp-scan-card">
                            ...
                            <button type="button" id="mp-back-btn" onclick="exitFullscreenCam()">← BACK</button>
                                ...
                                <div style="margin-top:24px; text-align:center;">
                                    <button type="button" id="mp-fsa-done-btn" onclick="closeFsaSection()" style="display:inline-block; padding:7px 22px; font-size:9px; font-weight:800; letter-spacing:1.2px; border-radius:30px; cursor:pointer; font-family:inherit; background:rgba(0,207,255,0.10); border:1px solid rgba(0,207,255,0.45); color:#00cfff;">
                                        ✓ DONE
                                    </button>
                                </div>
                                ...
                                <div style="margin:12px auto 0; max-width:300px; position:relative;">
                                    <input type="text" id="fbs-manual-input" oninput="this.value=this.value.toUpperCase()" placeholder="Or type UFC manually…"
                                           style="width:100%; text-transform: uppercase; box-sizing:border-box; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); border-radius:10px; color:#ccc; font-size:11px; padding:9px 36px 9px 12px; letter-spacing:0.5px; outline:none;">
                                    ...
                                    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:10px;">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" id="fbs-attr-brand" placeholder="e.g. Oakley, Ray-Ban…"
                                                   ...
                                                   style="width:100%; text-transform: uppercase; box-sizing:border-box; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); border-radius:10px; color:#ccc; font-size:11px; padding:9px 12px; letter-spacing:0.5px; outline:none;">
                                        </div>
                                ...
                                <div style="margin-top:24px; text-align:center;">
                                    <button type="button" id="fbs-done-btn" onclick="closeFbsSectionAndAdvance()" style="display:inline-block; padding:7px 22px; font-size:9px; font-weight:800; letter-spacing:1.2px; border-radius:30px; cursor:pointer; font-family:inherit; background:rgba(0,255,136,0.10); border:1px solid rgba(0,255,136,0.45); color:#00ff88;">
                                        ✓ DONE
                                    </button>
                                </div>
                                    ...
                                    <div id="cfr-body" style="display:none; margin-top:14px;">
                                        ...
                                        <div style="display:flex; flex-direction:column; gap:10px; text-align:left;">
                                            ...
                                            <div>
                                                ...
                                                <input type="text" id="cfr-brand" placeholder="E.G. BRENDEN, POLICE, SILHOUTTE…"
                                                       style="width:100%; text-transform: uppercase; box-sizing:border-box; background:rgba(255,255,255,0.04); border:1px solid rgba(255,138,77,0.35); border-radius:10px; color:#ccc; font-size:11px; padding:10px 12px; letter-spacing:0.5px; outline:none; font-family:inherit;"
                                            ...
                                            <div>
                                                ...
                                                <input type="text" id="cfr-price" placeholder="E.G. 350,000"
                                                       inputmode="numeric"
                                                       style="width:100%; text-transform: uppercase; box-sizing:border-box; background:rgba(255,255,255,0.04); border:1px solid rgba(255,138,77,0.35); border-radius:10px; color:#ffaa00; font-size:11px; padding:10px 12px; letter-spacing:0.5px; outline:none; font-family:monospace;"
                                            ...
                                            <div>
                                                ...
                                                <input type="text" id="cfr-size" placeholder="E.G. 52-18-140, 54-17"
                                                       inputmode="numeric"
                                                       style="width:100%; text-transform: uppercase; box-sizing:border-box; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.12); border-radius:10px; color:#ccc; font-size:11px; padding:10px 12px; letter-spacing:0.5px; outline:none; font-family:inherit;"
                                        ...
                                        <div style="margin-top:24px; text-align:center;">
                                            <button type="button" id="cfr-done-btn" onclick="closeCfrSectionAndAdvance()" style="display:inline-block; padding:7px 22px; font-size:9px; font-weight:800; letter-spacing:1.2px; border-radius:30px; cursor:pointer; font-family:inherit; background:rgba(255,138,77,0.10); border:1px solid rgba(255,138,77,0.45); color:#ff8a4d;">
                                                ✓ DONE
                                            </button>
                                        </div>
                            ...
                            <div class="prescription-container" style="border:1px solid rgba(255,170,0,0.18);">
                                ...
                                <div onclick="lrToggle()" style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;">
                                    <div style="display:flex;align-items:center;gap:10px;">
                                        <span style="font-size:1.25rem;">🔬</span>
                                        <div>
                                            <div style="font-size:0.7rem;letter-spacing:2px;color:#ffaa00;font-weight:700;display:flex;align-items:center;gap:8px;">
                                                LENS RECOMMENDED
                                                <button type="button" id="lr-done-btn" onclick="lrDoneAdvance(event)" style="font-size:7.5px;letter-spacing:1px;font-weight:700;color:#00ff88;background:rgba(0,255,136,0.08);border:1px solid rgba(0,255,136,0.35);border-radius:12px;padding:2px 8px;cursor:pointer;font-family:inherit;line-height:1.4;">✓ DONE</button>
                                            </div>
        ...
        <script>
            ...
            if (modNo) modNo.onclick = () => {
                ...
                // Advance: close PRESCRIPTION MODIFICATION, open CUSTOMER PURCHASE FRAME?
                closeModSection();
                openFpSection();
            };
            ...
            function setFramePurchase(val) {
                ...
                if (val === 1) {
                    ...
                    // Advance: close CUSTOMER PURCHASE FRAME?, open LENS RECOMMENDED
                    closeFpSectionOnly();
                    openLensRecSection();
                }
            }
            ...
            // ============================================================
            // AUTO-ADVANCE FLOW — card auto open/close + auto-scroll
            // ============================================================

            // Smooth-scroll a card into view (slight delay lets layout settle)
            ...
            // Close PRESCRIPTION MODIFICATION
            function closeModSection() {
                var panel = document.getElementById('mod-collapsible');
                var chev  = document.getElementById('mod-toggle-chev');
                if (panel) panel.style.display = 'none';
                if (chev)  chev.style.transform = 'rotate(0deg)';
            }
            window.closeModSection = closeModSection;

            // Open CUSTOMER PURCHASE FRAME?
            function openFpSection() {
                var panel = document.getElementById('fp-collapsible');
                var chev  = document.getElementById('fp-toggle-chev');
                if (panel) panel.style.display = 'block';
                if (chev)  chev.style.transform = 'rotate(180deg)';
                scrollToCard('frame-purchase-toggle-wrap');
            }
            window.openFpSection = openFpSection;

            // Close CUSTOMER PURCHASE FRAME? (panel only, keeps YES/NO sub-card state intact)
            function closeFpSectionOnly() {
                var panel = document.getElementById('fp-collapsible');
                var chev  = document.getElementById('fp-toggle-chev');
                if (panel) panel.style.display = 'none';
                if (chev)  chev.style.transform = 'rotate(0deg)';
            }
            window.closeFpSectionOnly = closeFpSectionOnly;

            // Close FACE SHAPE ANALYSIS only
            function closeFsaSection() {
                var body = document.getElementById('fsa-body');
                var chev = document.getElementById('fsa-chev');
                if (body) body.style.display = 'none';
                if (chev) chev.style.transform = 'rotate(0deg)';
                var switchBtn = document.getElementById('mp-switch-btn');
                if (switchBtn && switchBtn.style.display !== 'none' && typeof exitFullscreenCam === 'function') {
                    exitFullscreenCam();
                }
            }
            window.closeFsaSection = closeFsaSection;

            // Close FRAME BARCODE SCAN + advance: close CUSTOMER PURCHASE FRAME?, open LENS RECOMMENDED
            function closeFbsSectionAndAdvance() {
                var body = document.getElementById('fbs-body');
                var chev = document.getElementById('fbs-chev');
                if (body) body.style.display = 'none';
                if (chev) chev.style.transform = 'rotate(0deg)';
                if (typeof fbsStopCamera === 'function') fbsStopCamera();
                closeFpSectionOnly();
            ...
            window.closeFbsSectionAndAdvance = closeFbsSectionAndAdvance;

            // Close NEW FRAME INPUT + advance: close CUSTOMER PURCHASE FRAME?, open LENS RECOMMENDED
            function closeCfrSectionAndAdvance() {
            ...
            window.closeCfrSectionAndAdvance = closeCfrSectionAndAdvance;

            // Open LENS RECOMMENDED
            function openLensRecSection() {
                var body = document.getElementById('lr-body');
                var chev = document.getElementById('lr-chev');
                if (body) body.style.display = 'block';
                if (chev) chev.style.transform = 'rotate(180deg)';
                scrollToCard('lens-rec-wrap');
            }
            window.openLensRecSection = openLensRecSection;

            // DONE on LENS RECOMMENDED: close LENS RECOMMENDED + ORDER DETAILS, open CUSTOMER SELECTION
            function lrDoneAdvance(evt) {
                ...
                scrollToCard('lr-selection-bar');
            }
            window.lrDoneAdvance = lrDoneAdvance;

            // Flag: true when this page load is the reload right after saving a prescription modification
            var justSavedMod = <?php echo (isset($_GET['status']) && $_GET['status'] === 'success') ? 'true' : 'false'; ?>;
            ...
            window.onload = () => {
                ...
                if (isModified && modYes && modNo) {
                    if (!justSavedMod) {
                        // Normal reload of an already-modified invoice: auto-open the collapsible panel
                    ...
                    if (justSavedMod) {
                        // Just saved the modification: advance straight to CUSTOMER PURCHASE FRAME?
                        closeModSection();
                        openFpSection();
                    }
                }
        ...
        function openPrintPage() {
            ...
            // Custom (non-stock) frames use their brandKey as the frame_ufc
            // value as well (it does not exist in frames_main/frame_staging,
            // but customer_orders.frame_ufc should still store brand_key so
            // the order can be traced back to its custom_frames record).
            ...
            overlay.dataset.frameUfc   = frameUfc.toUpperCase();
